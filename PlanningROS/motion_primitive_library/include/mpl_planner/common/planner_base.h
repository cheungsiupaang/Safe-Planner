...
class PlannerBase {
 public:
  ...
  /// Set max vel in each axis
  void setVmax_quad(decimal_t v) {
    ENV_->set_v_max_quad(v);
    if (planner_verbose_) printf("[PlannerBase] set v_max_quad: %f\n", v);
  }

  /// Set max acc in each axis
  void setAmax_quad(decimal_t a) {
    ENV_->set_a_max_quad(a);
    if (planner_verbose_) printf("[PlannerBase] set a_max_quad: %f\n", a);
  }
