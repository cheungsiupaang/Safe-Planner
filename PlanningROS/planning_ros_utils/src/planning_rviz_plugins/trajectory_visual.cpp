...
void TrajectoryVisual::setMessage(const planning_ros_msgs::Trajectory &msg) {
  ...
  if (quad_pos_vis_)quad_poss_.resize(num_ - 1);
  if (quad_vel_vis_) quad_vels_.resize(num_);
  if (quad_acc_vis_) quad_accs_.resize(num_);
  if (quad_jrk_vis_) quad_jrks_.resize(num_);
  if (quad_cable_vis_) cable_.resize((num_-1)/10+1);
  ...
  const auto quad_waypoint = p.quad_sample(num_ - 1);
//   printf("load size : %i quad size: %i",waypoints.size(), quad_waypoint.size());
  ...
  for (unsigned int i = 0; i < quad_waypoint.size(); i++) {
    const auto p1 = quad_waypoint[i];
    const Ogre::Vector3 pos1(p1.pos(0), p1.pos(1), p1.pos(2));

    if (quad_pos_vis_){
    if (i < quad_waypoint.size() - 1) {
      const auto p2 = quad_waypoint[i + 1];
      const Ogre::Vector3 pos2(p2.pos(0), p2.pos(1), p2.pos(2));
      quad_poss_[i].reset(new rviz::BillboardLine(scene_manager_, frame_node_));
      quad_poss_[i]->addPoint(pos1);
      quad_poss_[i]->addPoint(pos2);
      }
    }
    ...
    if (quad_cable_vis_){
        if (i % 10 == 0){
            int k =i /10;
            const auto p4 = waypoints[i];
            const Ogre::Vector3 pos4(p4.pos(0), p4.pos(1), p4.pos(2));
            cable_[k].reset(new rviz::BillboardLine(scene_manager_, frame_node_));
            cable_[k]->addPoint(pos1);
            cable_[k]->addPoint(pos4);
        }
    }
    ...
    if (quad_vel_vis_) {
      const Vec3f p3 = p1.pos + R * p1.vel;
      const Ogre::Vector3 pos3(p3(0), p3(1), p3(2));
      quad_vels_[i].reset(new rviz::BillboardLine(scene_manager_, frame_node_));
      quad_vels_[i]->addPoint(pos1);
      quad_vels_[i]->addPoint(pos3);
    }

    if (quad_acc_vis_) {
      const Vec3f p3 = p1.pos + R * p1.acc;
      const Ogre::Vector3 pos3(p3(0), p3(1), p3(2));
      quad_accs_[i].reset(new rviz::BillboardLine(scene_manager_, frame_node_));
      quad_accs_[i]->addPoint(pos1);
      quad_accs_[i]->addPoint(pos3);
    }

    if (quad_jrk_vis_) {
      const Vec3f p3 = p1.pos + R * p1.jrk;
      const Ogre::Vector3 pos3(p3(0), p3(1), p3(2));
      quad_jrks_[i].reset(new rviz::BillboardLine(scene_manager_, frame_node_));
      quad_jrks_[i]->addPoint(pos1);
      quad_jrks_[i]->addPoint(pos3);
    }
  }
...
void TrajectoryVisual::setPosVis_quad(bool vis) { quad_pos_vis_ = vis; }

void TrajectoryVisual::setVelVis_quad(bool vis) { quad_vel_vis_ = vis; }

void TrajectoryVisual::setAccVis_quad(bool vis) { quad_acc_vis_ = vis; }

void TrajectoryVisual::setJrkVis_quad(bool vis) { quad_jrk_vis_ = vis; }

void TrajectoryVisual::setCableVis_quad(bool vis) { quad_cable_vis_ = vis; }
...
void TrajectoryVisual::setPosColor_quad(float r, float g, float b, float a) {
  //@note: for quad
  if(quad_pos_vis_)
  for (auto &it : quad_poss_) it->setColor(r, g, b, a);
}
...
void TrajectoryVisual::setVelColor(float r, float g, float b, float a) {
  ...
  //@note: for quad
  if (quad_vel_vis_)
  for (auto &it : quad_vels_) it->setColor(r, g, b, a);
}
...
void TrajectoryVisual::setAccColor(float r, float g, float b, float a) {
  ...
  //@note: for quad
  if (quad_acc_vis_)
  for (auto &it : quad_accs_) it->setColor(r, g, b, a);
}
...
void TrajectoryVisual::setJrkColor(float r, float g, float b, float a) {
  ...
  //@note: for quad
  if (quad_jrk_vis_)
  for (auto &it : quad_jrks_) it->setColor(r, g, b, a);
  if (quad_cable_vis_)
  for (auto &it : cable_) it->setColor(r, g, b, a);
}
...
void TrajectoryVisual::setPosScale(float s) {
  ...
  //@note: for quad
  if(quad_pos_vis_)
  for (auto &it : quad_poss_) it->setLineWidth(s);
}
...
void TrajectoryVisual::setVelScale(float s) {
  ...
  //@note: for quad
  if (quad_vel_vis_)
  for (auto &it : quad_vels_) it->setLineWidth(s);
}
...
void TrajectoryVisual::setAccScale(float s) {
  ...
  //@note: for quad
  if (quad_acc_vis_)
  for (auto &it : quad_accs_) it->setLineWidth(s);
}
...
void TrajectoryVisual::setJrkScale(float s) {
  ...
  //@note: for quad
  if (quad_jrk_vis_)
  for (auto &it : quad_jrks_) it->setLineWidth(s);
  if (quad_cable_vis_)
  for (auto &it : cable_) it->setLineWidth(s);
}
